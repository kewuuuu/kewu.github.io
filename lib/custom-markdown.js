...
var INLINE_TAGS = new Set(["span", "mark", "u", "sup", "sub", "anchor", "xref", "cell", "math", "hover"]);
...
function renderInlineTag(md, env, tagName, attrs, innerRaw) {
  ...
  if (tagName === "hover") {
    idValue = sanitizeId(attrs.id || attrs.ref || attrs.to || attrs.target || attrs.card);
    if (!idValue) {
      return md.renderInline(innerRaw, env);
    }
    innerHtml = md.renderInline(innerRaw, env);
    presentation = buildInlinePresentation(attrs, ["md-inline", "md-hover-term"]);
    tagAttrs = {
      class: presentation.className,
      style: presentation.style,
      "data-hover-id": idValue,
      tabindex: "0",
...
function renderHoverCardBlock(md, env, attrs, rawContent) {
  var hoverId = sanitizeId(attrs.id || attrs.ref || attrs.to || attrs.target || attrs.card) || nextGeneratedId(env, "hover");
  var className = sanitizeClassNames(attrs.class);
  var classes = "md-hover-card-def" + (className ? " " + className : "");
  var title = sanitizeText(attrs.title || attrs.summary || attrs.label || "");
  var width = sanitizeCssLength(attrs.width, false);
  var maxWidth = sanitizeCssLength(attrs.max || attrs.maxwidth || attrs["max-width"], false);
  var renderedTitle = title ? '<div class="md-hover-card-def__title">' + md.renderInline(title, env) + "</div>" : "";
  var renderedBody = '<div class="md-hover-card-def__body">' + md.render(rawContent, env) + "</div>";
  var styleParts = [];

  if (width) {
    styleParts.push("--md-hover-width:" + width);
  }
  if (maxWidth) {
    styleParts.push("--md-hover-max-width:" + maxWidth);
  }
  ...
  return (
    ...
    escapeHtml(hoverId) +
    '" aria-hidden="true"' +
    (styleParts.length ? ' style="' + escapeHtml(styleParts.join(";")) + '"' : "") +
    ">" +
    renderedTitle +
    renderedBody +
    "</section>"
...
function customDirectiveBlockRule(state, startLine, endLine, silent) {
  ...
  if (info.name !== "block" && info.name !== "details" && info.name !== "hovercard") {
    return false;
  }
  ...
  var renderedHtml = info.name === "block"
    ...
    : info.name === "details"
      ...
      : renderHoverCardBlock(state.md, state.env, info.attrs, content);
